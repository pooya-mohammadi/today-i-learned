...
8) mel-bands: 
   1) How many mel-bands? It depends on the problem. It's like a hyper-paramer
      1) 40, 60, 90, 128
   2) Number of mel-bands kind of references to the number of unique(not quite sure about this term) frequencies that are used in the input audio. Mainly, 40 distinct frequencies are used in western music.
9) How to construct mel-filter banks
   1) Convert lowest/highest frequency to Mel
      1) m = 2595 * log(1 + (f/500))
   2) Create # bands equally spaced points
      1) Now that we have the lowest and the highest mel-banks
      2) We want to equally split the distance to the number of mel-banks we want to use.
   3) Convert points back to Hertz :)
      1) f = 700(10^(m/2595) - 1)
   4) Round to the nearest frequency
   5) Create triangular filters
      1) ![img.png](images/wave/mel-triangular.png)
      2) In this image, we have six mel-bands
         1) ex: 1526 mel is equal to 2000 frequency Hz
      3) The image literally says, as we go further in terms of frequency, we should express more frequency to be perceptually understandable. This is the way we perceive peatch.
      4) The head of a triangle is one of the bands. The other two nodes of any triangle is the previous and subsequent mel-bands converted to frequency.
      5) Weights: is representative of how filter is going to be applied. Weights of 1.0 means zero filter bank
10) Mel filter banks' shape
    1) (#bands, framesize/2 +1)
11) Applying filter banks to spectrogram!
    1) M = (#bands, framesize/2 +1)
    2) Y = (freamesize/2 + 1, #frames) -> shape of a spectrogram!(nikoest frequency!)
    3) Mel Spectrogram = MY :) -> (#bands, #frames)
12) Mel-spectrogram:
    1) ![img.png](images/wave/mel-spec-sample.png)
    2) It's no different from spectrogram. Here each frequency bin is a different mel-band.
    3) The way we show frequency in this time is more relevant to human perceiving.
